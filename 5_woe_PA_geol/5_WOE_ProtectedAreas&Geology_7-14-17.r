...
SA_Recalc_Tables_Folder <-"V:/IaraSpatialLayers/Dinamica_Runs/StudyArea_V108/SA_V1081_NoRegion/Parameters/WOE_recalc/"

# ----------------------------------------------
#path to folder to write the edited weights of evidence tables
SA_Output_Folder <- "V:/IaraSpatialLayers/Dinamica_Runs/StudyArea_V108/SA_V1081_NoRegion/Parameters/WOE_recalc/LPA/"         
...
SA_recTable_Files<- list.files(SA_Recalc_Tables_Folder,pattern = ".*v108.*\\csv$")
...
# geology <- filter(sav108allWOE_sabuff, Variable.==" nlcdC_11/geology")
# Loudoun <- filter(geology, Din_cty=="rv108_WOE04")
# View(Loudoun)
# View(geology)
...
for(f in 1:length(SA_recTable_Files)){
  ...
  print(t)
	...
	try(WOE_pg[[t_t]][grepl("prot_gap_11", WOE_pg[[f]]$Variable.)& WOE_pg[[f]]$To.==6 & WOE_pg[[f]]$Range_Lower_Limit.==4,]$Weight<- 3)
	try(WOE_pg[[t_t]][grepl("prot_gap_11", WOE_pg[[f]]$Variable.)& WOE_pg[[f]]$To.==6 & WOE_pg[[f]]$Range_Lower_Limit.<4 & WOE_pg[[f]]$Range_Upper_Limit.<=4,]$Weight<- -3)
