...
###########################################
 ...
 PopProj<-read.csv("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/RegionalPopProjections/RegionalPopProjections_High_03_29_18.csv") # These are a 24% increase of the original population projections extrapolated from the counties.


 
# **!! NEED TO MAKE SURE ORDER IS BY YEAR.!!**  Easiest thing to do is to just sort csv.
...
system.time(out <- jags(jdata, inits, parameters, "lg.txt", parallel = T, #codaOnly = c('po'),
 ...
 print(out, digits = 3)
  save.image("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogGr_Region_24p_5-18-18a.RData")
...
# print(out1, digits = 3)
 ...
 # save.image("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogGr_Region_24p_5-18-18b.RData")
...
# write.csv(bind2, "U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs00_24p_5-18-18_b.csv", row.names = F)
...
# SCALING POPULATION
 ...
 Pop10 <- as.numeric(scale(prPop[9:16])) # 10 years out - Population for 2011. 
 Pop20 <- as.numeric(scale(prPop[17:24]))# 20 years out - Population for 2021. 
 Pop30 <- as.numeric(scale(prPop[25:32]))# 30 years out - Population for 2031. 
 Pop40 <- as.numeric(scale(prPop[33:40]))# 40 years out - Population for 2041.  
 Pop50 <- as.numeric(scale(prPop[41:48]))# 50 years out - Population for 2051.  
 Pop60 <- as.numeric(scale(prPop[49:56]))# 50 years out - Population for 2061. 
  


### READ FROM FILE ###
#10
# Nin<- read.csv("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs00_24p_5-18-18_b.csv") #Grab previous years output to form new baseline
...
Nt <- Nt1 <- N2011  # Create new data objects to overwrite with predicted data
 ...
 write.csv(bind2, "U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs10_24p_5-18-18_b.csv", row.names = F)
 
 
 #20
 # Population=as.numeric(scale(Pop20))  # Scale total population data (as done for the JAGS analysis)
 Nin<- read.csv("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs10_24p_5-18-18_b.csv") #Grab previous years output to form new baseline
 NinD<- Nin$PredPercDev
 Nt <- Nt1 <- NinD  # Create new data objects to overwrite with predicted data
 ...
 write.csv(bind2, "U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs20_24p_5-18-18_b.csv", row.names = F)
 
 #30
 # Population=as.numeric(scale(Pop30))  # Scale total population data (as done for the JAGS analysis)
 Nin<- read.csv("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs20_24p_5-18-18_b.csv") #Grab previous years output to form new baseline
 NinD<- Nin$PredPercDev
 Nt <- Nt1 <- NinD  # Create new data objects to overwrite with predicted data
...
write.csv(bind2, "U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs30_24p_5-18-18_b.csv", row.names = F)
 
 
 #40
 # Population=as.numeric(scale(Pop40))  # Scale total population data (as done for the JAGS analysis)
 Nin<- read.csv("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs30_24p_5-18-18_b.csv") #Grab previous years output to form new baseline
 NinD<- Nin$PredPercDev
 Nt <- Nt1 <- NinD  # Create new data objects to overwrite with predicted data
 ...
 write.csv(bind2, "U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs40_24p_5-18-18_b.csv", row.names = F)
 
 
 #50
 # Population=as.numeric(scale(Pop50))  # Scale total population data (as done for the JAGS analysis)
 Nin<- read.csv("U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs40_24p_5-18-18_b.csv") #Grab previous years output to form new baseline
 NinD<- Nin$PredPercDev
 Nt <- Nt1 <- NinD  # Create new data objects to overwrite with predicted data
 ...
 write.csv(bind2, "U:/CLI/Dinamica_Runs/Sub_Model Development/UrbanGrowthPatterns/LogRegOutputs/24perInc_5-18-18/LogRegOutputs50_24p_5-18-18_b.csv", row.names = F)
