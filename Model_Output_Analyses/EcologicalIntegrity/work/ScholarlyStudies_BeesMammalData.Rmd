...
b_dev250<-glm(data$SpRichness ~ data$dev_250, family="poisson" )
cov.b_dev250<-vcovHC(b_dev250, type="HC0")
std.err <- sqrt(diag(cov.b_dev250))
r.est <- cbind(Estimate= coef(b_dev250), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_dev250)/std.err), lower.tail=FALSE),
LL = coef(b_dev250) - 1.96 * std.err,
UL = coef(b_dev250) + 1.96 * std.err)

r.est

with(b_dev250, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Forest 250m 
```{r}
#Forest
...
b_for250<-glm(data$SpRichness ~ data$for_250, family="poisson" )
cov.b_for250<-vcovHC(b_for250, type="HC0")
std.err <- sqrt(diag(cov.b_for250))
r.est <- cbind(Estimate= coef(b_for250), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_for250)/std.err), lower.tail=FALSE),
LL = coef(b_for250) - 1.96 * std.err,
UL = coef(b_for250) + 1.96 * std.err)

r.est
with(b_for250, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Crop 250m 
```{r}
#Crop
...
b_cro250<-glm(data$SpRichness ~ data$cro_250, family="poisson" )
cov.b_cro250<-vcovHC(b_cro250, type="HC0")
std.err <- sqrt(diag(cov.b_cro250))
r.est <- cbind(Estimate= coef(b_cro250), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_cro250)/std.err), lower.tail=FALSE),
LL = coef(b_cro250) - 1.96 * std.err,
UL = coef(b_cro250) + 1.96 * std.err)

r.est
with(b_cro250, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Grass 250m 
```{r}
#Grass
...
b_gra250<-glm(data$SpRichness ~ data$gra_250, family="poisson" )
cov.b_gra250<-vcovHC(b_gra250, type="HC0")
std.err <- sqrt(diag(cov.b_gra250))
r.est <- cbind(Estimate= coef(b_gra250), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_gra250)/std.err), lower.tail=FALSE),
LL = coef(b_gra250) - 1.96 * std.err,
UL = coef(b_gra250) + 1.96 * std.err)

r.est
with(b_gra250, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
...
b_dev500<-glm(data$SpRichness ~ data$dev_500, family="poisson" )
cov.b_dev500<-vcovHC(b_dev500, type="HC0")
std.err <- sqrt(diag(cov.b_dev500))
r.est <- cbind(Estimate= coef(b_dev500), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_dev500)/std.err), lower.tail=FALSE),
LL = coef(b_dev500) - 1.96 * std.err,
UL = coef(b_dev500) + 1.96 * std.err)

r.est
with(b_dev500, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Forest 500m 
```{r}
#Forest
...
b_for500<-glm(data$SpRichness ~ data$for_500, family="poisson" )
cov.b_for500<-vcovHC(b_for500, type="HC0")
std.err <- sqrt(diag(cov.b_for500))
r.est <- cbind(Estimate= coef(b_for500), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_for500)/std.err), lower.tail=FALSE),
LL = coef(b_for500) - 1.96 * std.err,
UL = coef(b_for500) + 1.96 * std.err)

r.est
with(b_for500, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Crop 500m 
```{r}
#Crop
...
b_cro500<-glm(data$SpRichness ~ data$cro_500, family="poisson" )
cov.b_cro500<-vcovHC(b_cro500, type="HC0")
std.err <- sqrt(diag(cov.b_cro500))
r.est <- cbind(Estimate= coef(b_cro500), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_cro500)/std.err), lower.tail=FALSE),
LL = coef(b_cro500) - 1.96 * std.err,
UL = coef(b_cro500) + 1.96 * std.err)

r.est
with(b_cro500, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Grass 500m 
```{r}
#Grass
...
b_gra500<-glm(data$SpRichness ~ data$gra_500, family="poisson" )
cov.b_gra500<-vcovHC(b_gra500, type="HC0")
std.err <- sqrt(diag(cov.b_gra500))
r.est <- cbind(Estimate= coef(b_gra500), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_gra500)/std.err), lower.tail=FALSE),
LL = coef(b_gra500) - 1.96 * std.err,
UL = coef(b_gra500) + 1.96 * std.err)

r.est
with(b_gra500, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
...
b_dev1000<-glm(data$SpRichness ~ data$dev_1000, family="poisson" )
cov.b_dev1000<-vcovHC(b_dev1000, type="HC0")
std.err <- sqrt(diag(cov.b_dev1000))
r.est <- cbind(Estimate= coef(b_dev1000), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_dev1000)/std.err), lower.tail=FALSE),
LL = coef(b_dev1000) - 1.96 * std.err,
UL = coef(b_dev1000) + 1.96 * std.err)

r.est
with(b_dev1000, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
Bee & Forest 1km 
```{r}
#Forest
...
b_for1000<-glm(data$SpRichness ~ data$for_1000, family="poisson" )
cov.b_for1000<-vcovHC(b_for1000, type="HC0")
std.err <- sqrt(diag(cov.b_for1000))
r.est <- cbind(Estimate= coef(b_for1000), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_for1000)/std.err), lower.tail=FALSE),
LL = coef(b_for1000) - 1.96 * std.err,
UL = coef(b_for1000) + 1.96 * std.err)

r.est
with(b_for1000, cbind(res.foriance =foriance, df=df.residual, p=pchisq(foriance, df.residual, lower.tail=FALSE)))
```
Bee & Crop 1km
```{r}
#Crop
...
b_cro1000<-glm(data$SpRichness ~ data$cro_1000, family="poisson" )
cov.b_cro1000<-vcovHC(b_cro1000, type="HC0")
std.err <- sqrt(diag(cov.b_cro1000))
r.est <- cbind(Estimate= coef(b_cro1000), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_cro1000)/std.err), lower.tail=FALSE),
LL = coef(b_cro1000) - 1.96 * std.err,
UL = coef(b_cro1000) + 1.96 * std.err)

r.est
with(b_cro1000, cbind(res.croiance =croiance, df=df.residual, p=pchisq(croiance, df.residual, lower.tail=FALSE)))
```
Bee & Grass 1km 
```{r}
#Grass
...
b_gra1000<-glm(data$SpRichness ~ data$gra_1000, family="poisson" )
cov.b_gra1000<-vcovHC(b_gra1000, type="HC0")
std.err <- sqrt(diag(cov.b_gra1000))
r.est <- cbind(Estimate= coef(b_gra1000), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_gra1000)/std.err), lower.tail=FALSE),
LL = coef(b_gra1000) - 1.96 * std.err,
UL = coef(b_gra1000) + 1.96 * std.err)

r.est
with(b_gra1000, cbind(res.graiance =graiance, df=df.residual, p=pchisq(graiance, df.residual, lower.tail=FALSE)))
```
...
b_dev5000<-glm(data$SpRichness ~ data$dev_5000, family="poisson" )
cov.b_dev5000<-vcovHC(b_dev5000, type="HC0")
std.err <- sqrt(diag(cov.b_dev5000))
r.est <- cbind(Estimate= coef(b_dev5000), "Robust SE" = std.err,
"Pr(>|z|)" = 2 * pnorm(abs(coef(b_dev5000)/std.err), lower.tail=FALSE),
LL = coef(b_dev5000) - 1.96 * std.err,
UL = coef(b_dev5000) + 1.96 * std.err)

r.est
with(b_dev5000, cbind(res.deviance =deviance, df=df.residual, p=pchisq(deviance, df.residual, lower.tail=FALSE)))
```
